    ...
    def __init__(self, env):
        ...
        for s in states:
            for sp in states:
                ...
                self.expert._update_routes(tuple(s), tuple(sp))

    def pairwise_distances(self, indices, s0, s1):
        init_states = s0[indices]
        next_states = s1[indices]
        distances = [self.expert._get_distance(s, sp) for s, sp in zip(init_states, next_states)]
        return distances
...
if __name__ == '__main__':
    ...
    distances = [v[-1] for k, v in oracle.expert.saved_routes.items()]
