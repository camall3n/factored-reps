...
#%% ------------------ Load model ------------------
def torchify(x):
    return torch.as_tensor(np.moveaxis(x, -1, 0)).float().unsqueeze(0)

x = torchify(obs[0])
xp = torchify(next_obs[0])
fnet = FeatureNet(args, n_actions=5, input_shape=x.squeeze(0).shape, latent_dims=args.latent_dims)
fnet.load(model_file, to='cpu')

#%%
n_correct = 0
for x, a, xp in tqdm(zip(obs, actions, next_obs), total=len(states)):
    a_hat = fnet.predict_a(torchify(x), torchify(xp)).numpy()
    if a == a_hat:
        n_correct += 1
n_correct / len(states)

print(n_correct, 'correct out of', len(states), 'total = {}%'.format(100*n_correct/len(states)))
